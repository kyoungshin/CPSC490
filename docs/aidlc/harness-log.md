...
| 2026-09-18 | **Real:** a PR opened without `Closes #12` could never go green again. `pr-discipline` reads the PR body from the **event payload**; adding the line in the web UI fires no workflow event, and "Re-run failed jobs" replays the ORIGINAL payload with the stale body. Hit four student repos at once — and the instructor's own fix PRs | `pull_request` declares `types: [opened, synchronize, reopened, edited]`, so editing the body re-runs the check | `.github/workflows/ci.yml` | course scaffold |
| 2026-09-18 | **Real:** two student group repos (G02, G17) hit a RED harness on work they never wrote. `docs/sponsored-projects.md` — pure course reference — ended with a relative link to `../LICENSE`. Students correctly did NOT copy the instructor's all-rights-reserved LICENSE into their own repo, so G4 failed on a dangling link in a file they are not even meant to edit. Same class as the `example-*` first-push failure, reintroduced by a later page | Course-reference material never relative-links a repo-root file a student may legitimately not copy; such references use the absolute course-repo URL | `docs/sponsored-projects.md` | course scaffold |
